...
public class FloatingLock   :   MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D pick)
    {
        if (pick.CompareTag("Player"))
        {
            ...
            Debug.Log("Lock (Relic) has been picked up");
            SceneManager.LoadScene(30); //Scene change to lock claim cutscene, then boss fight
